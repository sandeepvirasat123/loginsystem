...
include("dbconnection.php");
		...
		if(isset($_POST['signup'])) {
			...
			//$ret= mysql_query("SELECT * FROM users ORDER BY id DESC LIMIT 0 , 1");
			//$num=mysql_fetch_array($ret);
			
			//$_SESSION['id']=$num['id'] + 1;
			...
			$a = date('Y-m-d');
			$msg = mysql_query("INSERT into users(fname,lname,DOB,email,password,usertype,posting_date,status) values('$fname','$lname','$dob','$email','$password','$type','$a','$status')");
			
			if($msg) {
	
			$ret= mysql_query("SELECT * FROM users WHERE email='".$_SESSION['login']."' and password='".$_SESSION['pass']."' and status != 'panding'");
			$num=mysql_fetch_array($ret);
				
				if($num>0) {
					$extra="edit_profile.php";
...
/* 					$extra="edit_profile.php";
		...
		if(isset($_POST['login'])) {
			...
			$password=$_POST['password'];
			$ret= mysql_query("SELECT * FROM users WHERE email='".$_POST['uemail']."' and password='$password' and status != 'panding'");
			$num=mysql_fetch_array($ret);
					...
					<div class="facts">
						...
						<div class="register">
							<form name="registration" method="post" action="" enctype="multipart/form-data">
								...
								<p>Which Type of User ?</p>
								<p><input type="radio" value="1" name="type"  checked>
								User Type 1&nbsp;&nbsp;
